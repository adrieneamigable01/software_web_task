...
ini_set('display_errors', 1); ini_set('display_startup_errors', 1); error_reporting(E_ALL); 
    ...
    if (!empty($result) ){
        ...
        $sql = "UPDATE enrollment SET student_id=? WHERE enrollment_id=?";
        ...
        ///Get Principal Email
        $sqlUsers = "SELECT * FROM `users` WHERE `user_type` like :principal_pattern AND users.is_active = :is_active";
        $queryUsers = $conn->prepare($sqlUsers);
        $queryUsers->execute([':principal_pattern' => 'Principal',':is_active' => 1]);
        $fetchUsers = $queryUsers->fetchAll(PDO::FETCH_ASSOC);
        sendAdminitratorEmail($fetchUsers[0]['email'],$fetchUsers[0]['user_type'], "Enrollment Notification",$data);
    ...
    function sendAdminitratorEmail($principal_email,$type,$subject,$data){
        ...
        send($principal_email,$subject,$email_body);
    }
